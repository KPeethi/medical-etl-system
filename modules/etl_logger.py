...
class ETLLogger:
    ...
    def __init__(self, run_type: str = "unknown", log_level: str = "INFO", source_path: str = "", max_log_size_mb: int = 50):
        ...
        self._logger = logging.getLogger(f"ETL.{run_type}")
        
        # Setup logging after logger is created
        self._setup_logging(log_level)
        
        # Log session start after logging is configured
    ...
    def _log_session_start(self):
        ...
        self._logger.info(f"Starting ETL session: {self.session_id}")
        self._logger.info(f"Run type: {self.run_type}")
        self._logger.info(f"Source path: {self.source_path}")
        ...
        try:
            if self.log_file_path.exists():
                ...
                if size_mb > self.max_log_size_mb:
                    ...
                    # Log the rotation
                    self._logger.info(f"Log rotated to part {self.current_log_part} (previous part was {size_mb:.1f}MB)")
                    
        except Exception as e:
            self._logger.warning(f"Error checking log size: {str(e)}")
    ...
    def log_file_processing_end(self, operation_id: str, result: Dict[str, Any]):
        ...
        if not operation:
            self._logger.error(f"Operation {operation_id} not found")
            return
    ...
    # Add property for backward compatibility with self._logger.logger pattern
    @property
    def logger(self):
        """Return the logger instance"""
        return self._logger if hasattr(self, '_logger') else None
    ...
    def log_patient_summary(self):
        ...
        self._logger.info("PATIENT PROCESSING SUMMARY")
        self._logger.info("=" * 80)
        
        total_patients = len(self.session_stats['patients_processed'])
        self._logger.info(f"Total Unique Patients Processed: {total_patients}")
        ...
        for method, count in self.session_stats['processing_methods'].items():
            if count > 0:
                ...
                self._logger.info(f"{method}: {count} files ({percentage:.1f}%)")
        
        # Log file type statistics
        self._logger.info("\nFILE TYPE BREAKDOWN:")
        for file_type, count in self.session_stats['file_type_counts'].items():
            percentage = (count / self.session_stats['total_files_processed']) * 100
            self._logger.info(f"{file_type}: {count} files ({percentage:.1f}%)")
        ...
        if self.session_stats['skipped_operations'] > 0:
            ...
            for reason, count in self.session_stats['skipped_reasons'].items():
                self._logger.info(f"  {reason}: {count} files")
    ...
    def finalize_session(self):
        ...
        self._check_log_size_and_rotate()
        self._logger.info("=" * 80)
        self._logger.info("ETL SESSION FINAL SUMMARY")
        self._logger.info("=" * 80)
        self._logger.info(f"Session ID: {self.session_id}")
        self._logger.info(f"Duration: {duration}")
        self._logger.info(f"Source Path: {self.source_path}")
        self._logger.info(f"Files Processed: {summary['total_files_processed']}")
        self._logger.info(f"Source Data Size: {summary['total_source_size_mb']:.2f}MB")
        self._logger.info(f"Destination Data Size: {summary['total_destination_size_mb']:.2f}MB")
        self._logger.info(f"Successful: {summary['successful_operations']}")
        self._logger.info(f"Failed: {summary['failed_operations']}")
        self._logger.info(f"Skipped: {summary['skipped_operations']}")
        self._logger.info(f"Unique Patients: {summary['unique_patients_processed']}")
        self._logger.info(f"Log Parts Created: {summary['log_parts_created']}")
        self._logger.info(f"Errors: {summary['total_errors']}")
        self._logger.info(f"Warnings: {summary['total_warnings']}")
    ...
    def _export_to_excel(self, summary: Dict[str, Any]):
        ...
        try:
            ...
            if success:
                self._logger.info(f"Excel log exported to: {excel_log_path}")
            else:
                self._logger.warning("Failed to export Excel log")
                
        except Exception as e:
            self._logger.warning(f"Excel export not available: {str(e)}")
    ...
    def _save_session_summary(self, summary: Dict[str, Any]):
        ...
        try:
            ...
            self._logger.info(f"Session summary saved to: {summary_file}")
            
        except Exception as e:
            self._logger.error(f"Error saving session summary: {str(e)}")
...
# ETL Logger module complete - no external integration dependencies
